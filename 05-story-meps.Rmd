...
| `press_disease` | 0 | 0 |
| `heart_disease` | 1 | 1 |
Table: Values of explanatory variables for both patients
...
As we can see, our explanation model tries to predict what would have happened if the patient would have a different sex And for the patient 1 there would be no difference in predicted value, but for patient 2 the change of sex implicates lowering or an increase in predicted costs. This means, that if there would exists a different patient with similar as patient 2 symptoms and attributes but with different sex, the predicted cost would be different.
...
On the plot above for variable `hosp_nights` both patients behave similary, but with different sensitivity. Patient 2, because of higher costs is more sensitive in changes of the number of nights associated with hospital discharges. We can notice here, that `hosp_nights` equal to 0 implicates that the expenses are also equal to 0. With the increase of `hosp_nights` the expected expenses also rise until value of `hosp_nights` equal to 100, then expenses are constant. It is worth mensioning that not always `hosp_nights` equal to 0 mean that there are no expenses.
...
For this patient our model had predicted expenses equal to 8779\$, where the real expenses were 2263\$. This means that for this observation our model overestimated the expenses by 6000$.
...
The first patient is of age 71, which in comparison to previous explanations should have significant impact on the prediction but not in this case. For this patient age increased the predicted expenses only by 141$. 

The biggest influence on the prediction has variable `phys_visits`, which is the number of office-based physician visits and it alone was responsible for 4300\$ of expenses. Also diabetes and an `feet_checked` (indicator whether the respondent reported having his or her feet checked for sores or irritations) have positive influence on the predicted expenses. These two observations increased the predicted cost bo 6000\$. 

The variables `hospital_nights`, `non-phys_visits` and `edu` have most significantly negative influence on the prediction and lowered the predicted expenses by 2400\$. The reason behind this might be that our observation has 0 `hospital_nights`.
...
We can run explanatory analysis using shaplay values for those patients. For Patient 1, all observations apart from `home_disease_days` have positive influence on the predicted value.

Top three variables with the biggest influence are diseases, and 3 after them are number of vists related variables. These top 3 diseases alone increased the predicted cost by 1800\$. Next 3 variables which indicated the Number of visits increased the expected cost by 1100\$. This is very interesting observation that is telling us, that diseases are the most important reasons why our medical expenses rise.
...
In the plot above, we can observe that for Patient 1 prediction is heavily influenced by `hosp_nights` which is the number of nights in a hospital and it influences negatively the prediction, thus lowering the expected costs. The same is for `home_disease_days` variable, which also negatively influenced the prediction. The reason behind such a result could be that our patient has 0 nights in hospital and 0 home days. On the other hand number of office-based physician visits has a positive impact on the models prediction, causing the expenses to be higher.

####  XIA for Patient 2

For this patient our model had predicted expenses equal to 24373\$, where the real expenses were 16268\$. This means that for this observation our model overestimated the expenses by 8000$.
...
This is quite interesing, because the variables that are making the expenses go higher are pure number of visits in hospital or by some medical employees. 
...
We can run explanatory analysis using shaplay values for those patients. For Patient 1, all observations have positive influence on the predicted value. 
...
As shown on the plot above, each value of the patient property influences a bit differently the outcome of our model. Patient 2, who has higher expenses, has higher values on our plots, but the dynamics of the plots for both patients are very similar.
